...
class ReflectionSyncWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {

    override suspend fun doWork(): Result {
        val db = LocalDatabase.getDatabase(applicationContext)
        val unsynced = db.reflectionDao().getUnsynced()
        ...
        unsynced.forEach { event ->
            eventsArray.put(JSONObject().apply {
                ...
                put("response", event.response)
                put("drift_score", event.driftScore)
        ...
        return try {
            ...
            if (success) {
                db.reflectionDao().markAsSynced(unsynced.map { it.id })
                Result.success()
            } else {
                Result.retry()
            }
        } catch (e: Exception) {
            Log.e("ReflectionSyncWorker", "Upload failed", e)
            Result.retry()
        }
    }

    private fun uploadBatch(userId: String, jwt: String, payload: JSONObject): Boolean {
        val url = URL("http://localhost:4000/v1/analytics/reflection/sync")
