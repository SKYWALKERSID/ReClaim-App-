...
class FrictionSyncWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {

    override suspend fun doWork(): Result {
        val db = LocalDatabase.getDatabase(applicationContext)
        val unsynced = db.frictionDao().getUnsynced()
        ...
        unsynced.forEach { event ->
            eventsArray.put(JSONObject().apply {
                ...
                put("friction_type", event.frictionType)
                put("drift_score", event.driftScore)
                put("overridden", event.isOverridden)
        ...
        return try {
            ...
            if (success) {
                db.frictionDao().markAsSynced(unsynced.map { it.id })
                Result.success()
            } else {
                Result.retry()
            }
        } catch (e: Exception) {
            Log.e("FrictionSyncWorker", "Upload failed", e)
            Result.retry()
        }
    }
