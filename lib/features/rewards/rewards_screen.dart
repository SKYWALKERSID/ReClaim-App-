        ...
        child: _isLoading 
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  const Text("Achievements", style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: -1)),
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _MetricTile(label: "Focus Points", value: "$_points", icon: Icons.bolt_rounded, color: AppColors.primary),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: _MetricTile(label: "Current Streak", value: "$_streak", icon: Icons.local_fire_department_rounded, color: Colors.orange),
                      ),
                    ],
                  ),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text("LEVEL $level", style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
              const Text("Pioneer", style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
            ],
          ),
          ...
          Text("${500 - (_points % 500)} points to Level ${level + 1}", style: const TextStyle(color: AppColors.textSecondary, fontSize: 11)),
        ],
      ),
    );
  }
      ...
      itemBuilder: (context, index) {
        return Column(
          children: [
            ...
            Text(_badges[index]['name'] ?? "Badge", style: const TextStyle(color: Colors.white70, fontSize: 10), textAlign: TextAlign.center),
          ],
        );
      },
    );
