...
import { dailyQuerySchema, rewardQuerySchema, } from "../validation/schemas.js";
import { resolvePathUserId } from "../utils/resolvePathUserId.js";
export function buildPolicyRoutes(repository) {
    const router = Router();
    /**
     * @openapi
     * /policy/{userId}:
    ...
    router.get("/policy/:userId", async (request, response, next) => {
        try {
            ...
            const metrics = (await repository.getDailyMetrics(userId, dateKey)) ?? {
                userId,
                dateKey,
