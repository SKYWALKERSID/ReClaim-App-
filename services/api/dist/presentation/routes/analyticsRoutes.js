...
import { z } from "zod";
import { eventsPayloadSchema, dailyQuerySchema, weeklyQuerySchema, deviceRegistrationSchema, } from "../validation/schemas.js";
import { rateLimiter } from "../middleware/rateLimiter.js";
import { resolvePathUserId } from "../utils/resolvePathUserId.js";
export function buildAnalyticsRoutes(service) {
    const router = Router();
    ...
    router.post("/analytics/events", ingestionLimiter, async (request, response, next) => {
        try {
            ...
            if (request.user.role === "service") {
                const ids = new Set(events.map((e) => e.userId).filter((x) => typeof x === "string" && x.length > 0));
                if (ids.size !== 1) {
                ...
                const uid = [...ids][0];
                toIngest = events.map((e) => ({ ...e, userId: uid }));
            }
            else {
    ...
    router.get("/export/:userId", async (request, response, next) => {
        try {
            ...
            const events = await service.getExportData(userId, from, to);
            if (format === "csv") {
                const csv = service.convertToCsv(events);
