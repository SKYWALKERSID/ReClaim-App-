...
export function buildPatternInsights(input) {
    ...
    const totalHoursTracked = Math.max(1, (Date.parse(usageEvents.at(-1)?.endedAt ?? new Date().toISOString()) -
    ...
    const riskScore = Math.min(100, Math.round(distractionMinutes * 0.8 +
